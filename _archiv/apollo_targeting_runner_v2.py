...
def score_organization(org: dict[str, Any], profile: SearchProfile) -> dict[str, Any]:
    """Bewertet eine Apollo-Firma defensiver als v1.

    Wichtige Änderung:
    - Fehlende Apollo-Beschreibung ist KEIN Ausschlussgrund.
    - Pharma-/Arzneimittel-Hersteller ohne weitere Daten werden als "P" = Prüf-Kandidat markiert.
    - D/Ausschluss gibt es nur bei echten Negativsignalen oder klar falschem Fit.
    """
    text_parts = [
        org.get("name", ""),
    ...
    # Harte Hersteller-/Pharma-Indikatoren: nicht genug für A/B, aber genug für "prüfen".
    manufacturer_signals = [
        "pharma", "arzneimittel", "otc", "rx", "medizinprodukte",
        "pharmaceutical", "hersteller", "manufacturer", "gesundheit"
    ]
    manufacturer_hits = [kw for kw in manufacturer_signals if kw in text_low]

    # Apotheken-/Direktvertriebssignale bekommen mehr Gewicht als allgemeines Pharma.
    strong_fit_signals = [
        "apotheke", "apotheken", "apothekenvertrieb", "direktbestellung",
        "direktvertrieb", "pzn", "bestellformular", "außendienst",
        "aussendienst", "pharmacy", "pharmacies", "field sales", "order form"
    ]
    strong_hits = [kw for kw in strong_fit_signals if kw in text_low]

    score = 0
    score += min(len(plus_hits) * 10, 60)
    score += min(len(strong_hits) * 18, 60)
    score += 25 if manufacturer_hits else 0
    score -= min(len(minus_hits) * 25, 80)
    ...
    has_meaningful_data = bool((org.get("short_description") or org.get("seo_description") or org.get("industry") or org.get("keywords")))
    ...
    if score >= 80:
        cls = "A"
        rec = "kontaktieren"
        next_step = "Ansprechpartner suchen und Ansprache vorbereiten"
    elif score >= 60:
        cls = "B"
        rec = "prüfen/kontaktieren"
        next_step = "Website prüfen und Ansprechpartner validieren"
    elif score >= 40:
        cls = "C"
        rec = "prüfen"
        next_step = "Apotheken-/Direktvertriebssignale auf Website prüfen"
    elif manufacturer_hits and not minus_hits:
        # Genau der Klinge-Pharma-Fall: Apollo liefert zu wenig Inhalt, aber der Firmenfit ist plausibel.
        cls = "P"
        rec = "Prüfkandidat"
        next_step = "Website manuell/automatisch prüfen; nicht ausschließen"
        score = max(score, 35)
    elif not has_meaningful_data and not minus_hits:
        cls = "U"
        rec = "unklar"
        next_step = "Apollo-Daten unvollständig; Website prüfen, bevor ausgeschlossen wird"
    else:
        cls = "D"
        rec = "ausschließen"
        next_step = "nicht priorisieren"

    positive = plus_hits + [f"Hersteller/Pharma-Indiz: {x}" for x in manufacturer_hits[:3]] + [f"Starkes Fit-Signal: {x}" for x in strong_hits[:5]]

    return {
        "score": score,
        "klasse": cls,
        "positive_signale": "; ".join(dict.fromkeys(positive)),
...
def build_result_row(profile: SearchProfile, org: dict[str, Any], person: dict[str, Any] | None, scoring: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "naechster_schritt": scoring.get("naechster_schritt", "Website prüfen"),
        "quelle": "Apollo/API",
    }
...
def main() -> None:
    ...
    for profile in profiles:
        ...
        for org in orgs:
            ...
            people = []
            if org_score["klasse"] in ["A", "B", "C", "P", "U"] and role_keywords:
